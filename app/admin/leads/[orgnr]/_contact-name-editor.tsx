...
export function ContactNameEditor({ orgNr, initialName }: Props) {
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        toast.success(next ? "Navn lagret" : "Navn fjernet");
        setEditing(false);
      } else {
    ...
    return (
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs text-muted-foreground">Kontaktperson</div>
          <div className="flex items-center gap-2 group">
            ...
            <button
              ...
              aria-label="Rediger kontaktperson"
            >
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <p className="text-[11px] text-muted-foreground">
          Brukes som «Hei {`{{contact_first_name}}`}» i e-postmaler.
        </p>
      </div>
    </div>
  );
